...
describe("package export and runtime contract", () => {
  ...
  it("allows only the exact locked native PTY package build", async () => {
    ...
    expect(optionalDependencies["node-pty"]).toBe("1.2.0-beta.12");
    expect(lockfile).toMatch(
      /node-pty:\n\s+specifier: 1\.2\.0-beta\.12\n\s+version: 1\.2\.0-beta\.12/
    );
