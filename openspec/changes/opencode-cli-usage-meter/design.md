...
| Claude process | Run plain-pipe `claude /usage` first, then dynamically load exact optional `node-pty@1.2.0-beta.12` on Linux/macOS when pipe output cannot be parsed. Missing native support returns unavailable. | Preserves the minimal path while supporting real CLI TTY requirements without making package import depend on a native addon. |
| Native build policy | Permit lifecycle scripts for exact `node-pty` only, preserving frozen-lock integrity and the existing `esbuild`/`msgpackr-extract` allowances. Do not repair package modes with CI `chmod`. | Version 1.1.0 published its macOS arm64 `spawn-helper` without executable mode. The exact beta publishes arm64/x64 helpers as `0755`; pinning bounds added script authority while accepting explicit prerelease supply-chain risk. |
...
- The exact beta corrects inspectable Darwin helper modes, but prerelease stability and future upstream republishing remain supply-chain risks controlled by the lockfile integrity.
