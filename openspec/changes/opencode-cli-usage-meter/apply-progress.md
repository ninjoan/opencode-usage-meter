...
- Added an explicit built-package import smoke script and wired it into CI immediately after `pnpm build`.
...
- [x] 1.4 REFACTOR: wired pnpm scripts, strict NodeNext checks, tsup build, explicit built-package smoke, CI frozen install, audit, and pack dry run. Strict-TDD RED is inherited from the package-contract work unit; this task is script/CI refactor evidence.
...
| Runtime harness command/scenario and exact result | `pnpm build && pnpm smoke:built-package` → exit 0; tsup built `dist/tui.js`, `dist/tui.js.map`, `dist/tui.d.ts`; `node -e "await import('./dist/tui.js')"` succeeded through the explicit script. |
...
| Correction | Proof |
|---|---|
| CI built-package smoke after build | `.github/workflows/ci.yml` validates `pnpm typecheck && pnpm test && pnpm build && pnpm smoke:built-package && pnpm audit:prod && pnpm pack:dry-run`; local `pnpm build && pnpm smoke:built-package` passed. |
...
| `pnpm test` | exit 0; 3 files passed, 8 tests passed |
| `pnpm typecheck` | exit 0; `tsc --noEmit` |
| `pnpm exec tsc --noEmit --skipLibCheck false` | exit 0; no diagnostics |
| `pnpm build && pnpm smoke:built-package` | exit 0; tsup ESM+DTS build and built import smoke passed |
